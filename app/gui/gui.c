...
{ 
  ...
  TFT_SetWindow(xState, yState, xEnd, yEnd); 
		LCD_SetCursor(xState,yState);           //设置光标位置  
	  LCD_WR_REG(0x22);                   //开始写入GRAM 
...
/*****************************************************************************
** Function name:       void LCD_DrawCircle(uint16_t x0, uint16_t y0, uint8_t r,uint16_t Color)
** Descriptions:        画圆
...
{
	GUI_Line(x-12,y,x+13,y,Color);              //横线
	GUI_Line(x,y-12,x,y+13,Color);              //竖线
                     ...
                     uint16_t wordColor, uint16_t backColor)
{
 ...
 LCD_SetCursor(x,y);           //设置光标位置  
	...
	TFT_SetWindow(0x0000,0x0000,240,320);	         //恢复窗体大小
}
